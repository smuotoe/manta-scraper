...
class Manta:
    def __init__(self, city_url=None, urls=None, site_num=None):
        self.city_url = city_url
        self.urls = urls
        self.site_num = site_num
        self.driver = webdriver.Chrome("C:\\Users\\tempSomto\\chromedriver.exe")


    def manta_scraper(self):
        count = 1
        for url in tqdm(self.urls[self.site_num:], desc="First loop"):
            ...
            try:
                self.driver.find_element_by_css_selector(".w30 .text-muted").click()
            except:
                try:
                    self.driver.find_element_by_css_selector('body').send_keys(Keys.ARROW_DOWN)
                    self.driver.find_element_by_css_selector('body').send_keys(Keys.ARROW_DOWN)
                    self.driver.find_element_by_css_selector(".w30 .text-muted").click()
            ...
            if num_of_pages > 1:
                for page_num in tqdm(range(num_of_pages), desc = "Second loop"):
                    ...
                    if page_num == 0:
                        ...
                        self.driver.get(url + "?pg=" + str(page_num+1))
                        page_soup = soup(self.driver.page_source, "html.parser")
                        try:
                           p = [i.attrs["content"] for i in page_soup.findAll("meta", {"itemprop":"url"})]
                        except:
